...
class Layer {
    constructor(mapa, geometries, clase, ids) {

        let root_map = mapa.mapa;
        this.geometries = geometries;
        this.clase = clase;
        this.normalizador = null;

        this.properties_map = new Map();

        geometries.features.forEach((mun) => {
            this.properties_map.set(mun.properties[ids],mun.properties);
        });
    ...
    sum_prop(propiedad) {
        let total = 0;
        this.properties_map.forEach((mun) => {
            if (mun.hasOwnProperty(propiedad)) {
                total += Number(mun[propiedad]);
            }
        });
...
function bbox(features) {
    if (typeof turf === 'undefined') {
        throw new Error("Turf no existe, carga turf antes de usar esta función");
    };

    if (typeof L === 'undefined') {
        throw new Error("Leaflet no existe, carga Leaflet antes de usar esta función");
    };
